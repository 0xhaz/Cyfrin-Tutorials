...
// 2. Run `forge script script/MakeMerkle.s.sol` to generate the merkle proof
// 3. The output file will be generated at /script/target/output.json

contract MakeMerkle is Script, ScriptHelper {
    using stdJson for string; // enables us to use the json cheatecodes for strings
    ...
    function run() public {
        ...
        for (uint256 i = 0; i < count; ++i) {
            ...
            bytes32[] memory data = new bytes32[](types.length); // data in bytes32 format
            ...
            for (uint256 j = 0; j < types.length; ++j) {
                if (compareStrings(types[j], "address")) {
                    ...
                    // uint160 (20 bytes) cast up to uint256 which is 32 bytes and then cast to bytes32
            ...
            // ltrim64 removes the offset and length from the encoded bytes. There is an offset because the array is
            // declared in memory
        ...
        for (uint256 i = 0; i < count; ++i) {
            ...
            // get the singified input (address and amount)
        ...
        // write to the output file the stringified output json tree dumps
